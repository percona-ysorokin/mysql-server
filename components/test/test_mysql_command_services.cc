...
static char *test_mysql_command_services_udf(UDF_INIT *, UDF_ARGS *args,
  ...
  *error = 1;
  ...
  if (args->arg_count > 1U && args->arg_type[1] == INT_RESULT) {
    long long number_of_query_executions_raw;
    std::memcpy(&number_of_query_executions_raw, args->args[1], sizeof number_of_query_executions_raw);
    long long constrained_number_of_query_executions_raw{std::clamp(number_of_query_executions_raw, 1LL, 10LL)};
    number_of_query_executions = static_cast<std::size_t>(constrained_number_of_query_executions_raw);
  }
